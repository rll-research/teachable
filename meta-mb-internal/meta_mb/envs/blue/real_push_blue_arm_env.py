...
class ArmPushEnv(MetaEnv, BlueInterface, gym.utils.EzPickle):
    def __init__(self, goal_dist=0.1, side='right', ip='127.0.0.1', port=9090):
        max_torques = np.array([5, 5, 4, 4, 3, 2, 2])
        self.frame_skip = 1
        self.dt = 0.2
        super(ArmPushEnv, self).__init__(side, ip, port)
    ...
    def _get_obs(self):
        #Still need to add the location of goal and peg insertion
    ...
    def reset(self):
        ...
        while True:
            self.goal_obj = np.zeros(3) #CHANGE THIS
            self.goal_dest = np.zeros(3) #CHANGE THIS
    ...
    def vec_gripper_to_goal(self):
        ...
        if not self.holding_obj:
            vec = gripper_pos - self.goal_obj
        else:
            vec = gripper_pos - self.goal_dest
