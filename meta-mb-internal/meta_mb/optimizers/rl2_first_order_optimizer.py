...
class RL2FirstOrderOptimizer(RNNFirstOrderOptimizer):
    ...
    def compute_loss_variations(self, input_dict, entropy_loss, reward_loss):
        ...
        kl_loss = sess.run(entropy_loss, feed_dict=feed_dict)
        low_reward_loss = sess.run(reward_loss, feed_dict=feed_dict)
        return kl_loss, low_reward_loss
