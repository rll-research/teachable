...
class DICEMAML(MAMLAlgo):
    ...
    def _make_dice_input_placeholders(self, prefix=''):
        ...
        for task_id in range(self.meta_batch_size):
            ...
            # reward ph
            ph = tf.placeholder(dtype=tf.float32, shape=[None, self.max_path_length], name='rewards' + '_' + prefix + '_' + str(task_id))
            all_phs_dict['%s_task%i_%s' % (prefix, task_id, 'rewards')] = ph
            reward_phs.append(ph)

            # baseline ph
            ph = tf.placeholder(dtype=tf.float32, shape=[None, self.max_path_length], name='baselines' + '_' + prefix + '_' + str(task_id))
...
def magic_box_cumsum(logprobs):
    ...
    with tf.variable_scope("magic_box_cumsum"):
