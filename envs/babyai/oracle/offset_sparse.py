...
class OFFSparse(Teacher):
    ...
    def step_ahead(self, oracle, last_action=-1):
        ...
        self.next_state, next_coords, actions, env = self.step_away_state(oracle, self.cartesian_steps,
    ...
    def feedback_condition(self, env):
        ...
        """
        if self.steps_since_lastfeedback % self.feedback_frequency == 0 or np.array_equal(env.agent_pos, self.goal_coords):
    ...
    def get_last_feedback_indicator(self):
        try:
            ...
            print("uh oh, looks like we can't choose a feedback freq (OffsetSparse)", e)
        return vec