...
class BaseAlgo(ABC):
    ...
    def collect_experiences(self, collect_with_oracle=False, collect_reward=True, train=True):
        ...
        for e in self.env.envs:
            print(e._wrapped_env.__class__.__name__)
        for i in range(self.num_frames_per_proc):
            ...
            if i < 10:
                print("Taking actions", i, action.cpu().numpy(), self.env.get_teacher_action())
            #else:
            #    assert False
            #collect_with_oracle = True
            #action = torch.IntTensor(np.stack(self.env.get_teacher_action())).to(self.device).unsqueeze(1)
            #action = agent_dict['argmax_action'].reshape(action.shape)
            ...
            for i, done_ in enumerate(done):
                if done_:
                    if self.log_episode_success[i].item():
                        print("success", i, self.log_episode_success[i].item())
                    else:
                        print("failure", i)
        ...
        keep = 50
        print("keep", self.log_success[-keep:])
